...
class Coupons:
    ...
    def obtain_available_coupons(self,
        ...
        pipeline.append({'$match': {'$or': [{'category_rules': {'$exists': False}}, {'category_rules': None}, {'category_rules': {'$in': [category]}}]}})

        # Filter by service
        # If the coupon has no service rules, it is valid for all services
        pipeline.append({'$match': {'$or': [{'service_rules': {'$exists': False}}, {'service_rules': None}, {'service_rules': {'$in': [service_id]}}]}})

        # Filter by provider
        # If the coupon has no provider rules, it is valid for all providers
        pipeline.append({'$match': {'$or': [{'provider_rules': {'$exists': False}}, {'provider_rules': None}, {'provider_rules': {'$in': [provider_id]}}]}})
    ...
    def add_user_to_coupon(self, coupon_code: str, user_id: str) -> bool:
        ...
        try:
            result = self.collection.update_one({'uuid': coupon_code}, {'$set': {'used_by': used_by}})
            return result.modified_count > 0
        except Exception as e:
            logger.error(f"Error adding user '{user_id}' to coupon '{coupon_code}': {e}")
            return False
    ...
    def add_item_to_rule(self, coupon_code: str, rule: str, item: str) -> bool:
        ...
        if not coupon or rule not in coupon or len(coupon[rule] or []) == 0:
            return False
        try:
            result = self.collection.update_one({'uuid': coupon_code}, {'$push': {rule: item}})
            return result.modified_count > 0
        except Exception as e:
